...
(function (window, document, $, undefined) {
    ...
    var _messages = {
        'default': '$ contiene uno o más errores.',
        'NOTEMPTY': '$ no debe estar vacio.',
    ...
    $.fn.validateForm = function () {
        return _api.validateForm(this);
    }
    $.fn.displayErrors = function () {
        return _api.displayErrors(this);
    }
    $.fn.validateInput = function (input) {
        return _api.validateInput(this, input);
    }

    $.fn.displayOneError = function (input) {
        return _api.displayOneError(this, input);
    }
    ...
    var _api = {
        ...
        validate: function (node, options) {
            ...
            if (options === "destroy") {
                ...
                if (!window.Validation.form[node]) {
                    window.Debug.log({
                        'node': node,
                        'function': '$.validate("destroy")',
                        'arguments': '',
                        'message': 'Unable to destroy "' + node + '", perhaps it\'s already destroyed?'
                    });
                ...
                window.Validation.form[node].destroy();
            ...
            return node.each(function () {
                window.Validation.form[node] = new Validation($(this), options);
            });
        ...
        addError: function (node, error) {
            if (!window.Validation.form[node]) {
                window.Debug.log({
                    'node': node,
                    'function': '$.addError()',
                    'arguments': 'window.Validation.form[' + node + ']',
            ...
            for (var inputName in error) {
                ...
                if (!input[0]) {
                    window.Debug.log({
                        ...
                        'function': '$.addError()',
                        'arguments': inputName,
                        'message': 'ERROR - Unable to find ' + '$(' + node + ').find("[name="' + inputName + '"]")'
                    });
                ...
                window.Validation.form[node].resetOneError(inputName, input);
                ...
                for (var i = 0; i < error[inputName].length; i++) {
                    ...
                    window.Validation.form[node].registerError(inputName, error[inputName][i]);

                }

                window.Validation.form[node].displayOneError(inputName);
        ...
        removeError: function (node, inputName) {
            ...
            if (!inputName) {
                window.Validation.form[node].resetErrors();
                return false;
            }
            ...
            for (var i = 0; i < inputName.length; i++) {
                ...
                if (!input[0]) {
                    window.Debug.log({
                        ...
                        'message': 'ERROR - Unable to find ' + '$(' + node + ').find("[name="' + inputName[i] + '"]")'
                    });
                ...
                window.Validation.form[node].resetOneError(inputName[i], input);
        ...
        validateForm: function (node) {
            var validation = window.Validation.form[node];
            return validation.validateForm();
        },
        validateInput: function (node, input) {
            var validation = window.Validation.form[node];
            return validation.validateInput(input);
        },
        displayErrors: function (node) {
            var validation = window.Validation.form[node];
            validation.displayErrors();
        },
        displayOneError: function (node, input) {
            var validation = window.Validation.form[node];
            validation.displayOneError(input);
        }
    };
    ...
    window.Debug = {
        ...
        log: function (debugObject) {
            ...
            this.table[debugObject.message] = debugObject;
