...
{
    public class ConvertNumberToLetterBL
    {
    ...
    {
        public string InicialMayuscula(string cadena)
        {
            string mayuculaMinuscula = "";
            if (!string.IsNullOrEmpty(cadena))
            {
                mayuculaMinuscula = cadena.Trim().ToLower();
                if (mayuculaMinuscula.Length > 0) mayuculaMinuscula = mayuculaMinuscula.Substring(0, 1).ToUpper() + mayuculaMinuscula.Substring(1);
            }
            return mayuculaMinuscula;
        }

        public string FormatText(string textoOriginal)
        {
            string textoFinal = textoOriginal;
            if (!string.IsNullOrEmpty(textoOriginal))
            {
                textoFinal = textoFinal.Trim().ToUpper();
                textoFinal = textoFinal.Replace("A", "Á");
                textoFinal = textoFinal.Replace("E", "É");
                textoFinal = textoFinal.Replace("I", "Í");
                textoFinal = textoFinal.Replace("O", "Ó");
                textoFinal = textoFinal.Replace("U", "Ú");
            }
            return textoFinal;
        }
    }
